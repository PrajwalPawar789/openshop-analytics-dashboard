    ...
    <div className="w-full bg-white p-4 rounded-sm border border-gray-200">
      ...
      <div className="mt-4 space-y-3">
        {popularProducts.map((product) => (
          <Link key={product.id} to={`/product/${product.id}`} className="flex items-start">
            <div className="w-16 h-16 min-w-16 bg-gray-200 rounded-sm overflow-hidden">
              <img
            ...
            <div className="text-sm text-gray-400">{product.product_price}</div>
          </Link>
        ))}
      </div>
    </div>
